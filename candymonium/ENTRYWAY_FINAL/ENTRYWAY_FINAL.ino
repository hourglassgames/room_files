...
void subscriptions(){
  ...
  client.subscribe("Candymonium Entryway Code");
  client.subscribe("Candymonium Entryway Status");
...
char* master_2 = "9984";
char* master_3 = "0368";
char current_entry[5];
int blacklight_status = 0;
...
void loop() {
  ...
  doorbellStatus = digitalRead(doorbellPin);
  if (doorbellStatus == LOW){
    client.publish("Candymonium Audio Buttons", "doorbell");
    delay(500);
  }
  ...
  if ((master_solved[0] == 1) && (master_solved[1] == 1) && (blacklight_status == 0)){
    client.publish("Candymonium ESP32 Lights", "blacklight_on");
    blacklight_status = 1;
  }
  ...
  msg_string.toCharArray(msg,50);
  client.publish("Candymonium Entryway Status",msg);
  client.loop();
  }
  ...
  if ((master_solved[0] == 1) && (master_solved[1] == 1) && (master_solved[2] == 1)){
    ...
    msg_string.toCharArray(msg,50);
    client.publish("Candymonium Entryway Code",msg);
    /*while (true){
      if (!client.connected()) {
        reconnect();
      }
