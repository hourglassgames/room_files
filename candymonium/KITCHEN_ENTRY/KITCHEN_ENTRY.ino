...
char msg[50];
int value = 0;
const char* device = "Candymonium ESP32 Kitchen Entry";//what you want to call this device
...
void subscriptions(){
  ...
  client.subscribe("Candymonium Kitchen Code");
  client.subscribe("Candymonium Kitchen Status");
}
...
void loop() {
  ...
  //while(customKeypad.available()){
    ...
    if(e.bit.EVENT == KEY_JUST_PRESSED){
      ...
      client.publish("Candymonium Kitchen Code",current_entry);
  ...
  if (codeCount == 5){
    if(!strcmp(master_1,current_entry)){
      ...
      //master_solved[0] = 1;
      digitalWrite(mosfetPin[0,1], HIGH);
      ...
      msg_string.toCharArray(msg,50);
      client.publish("Candymonium Kitchen Code",msg);
      //clearData();
      while (true){
      if (!client.connected()) {
        reconnect();
  ...
  //msg_string.toCharArray(msg,50);
  //client.publish("Candymonium Kitchen Status",msg);
  client.loop();
  }
/*
